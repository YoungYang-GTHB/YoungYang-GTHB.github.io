...
\begin{itemize}
  \item \textbf{Programming}: C/C++ (proficient), Python (fluent), MATLAB (fluent); Linux driver/application development
  \item \textbf{Robotics}: ROS/ROS2 (proficient); navigation, SLAM, motion planning, inverse kinematics
  \item \textbf{Embedded}: STM32/51/Arduino (proficient); development on IMX6ULL, STM32F4
  \item \textbf{Deep Learning}: PyTorch — image processing, speech recognition, CNN design
  \item \textbf{Hardware}: PCB design (Altium Designer, JLCPCB), 3D-printing modeling/slicing
  \item \textbf{Tools}: Git, PyQt5 (host software), Autodesk Inventor (3D modeling)
\end{itemize}

% ====================== Experience ======================
\section{Experience}

\ResumeItem{视启未来 (incubated by IDEA Research)}
