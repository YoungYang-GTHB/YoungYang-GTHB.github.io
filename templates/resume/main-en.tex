...
\begin{itemize}
  ...
  \item Built an AWBC / RECAP-advantage offline policy-improvement pipeline (offline MAE@1: 0.0079 vs. SFT 0.0089); ran cross-embodiment co-training and weight-space model-merging experiments
  \item Brought up Tsinghua's X-VLA architecture on our dual-arm platform, using firmware-level IK to cut the control cycle from 1.4 s to 0.33 s
  \item Built CRAVE, a training-free milestone/value engine that derives milestone and progress/value signals from frozen visual features (0.865 correlation with human stage-progress labels); explored three-level advantage conditioning for AWBC to reduce manual stage annotation
\end{itemize}
