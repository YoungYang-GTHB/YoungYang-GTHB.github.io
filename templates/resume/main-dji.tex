...
% ====================== 简介 ======================
\noindent 西北工业大学控制工程专业硕士在读（2027年6月毕业），方向机器人与人工智能。具备覆盖\hl{算法研发与部署、软件开发、电路设计与嵌入式系统开发}的全流程研发能力；围绕 \hl{VLA（视觉-语言-动作模型）与世界动作模型（WAM）}，积累了双臂机器人柔性操作策略的\hl{训练、实时推理优化与真机部署}经验。熟悉 ROS、Linux 应用开发与深度学习，参与\hl{国家级重点课题}，累计 \hl{6 项专利（4 发明 / 2 实用新型）}与 \hl{2 项软件著作权}。
...
\section{专业技能}
\begin{itemize}
  \item \textbf{AI / 具身智能}: \hl{VLA}（\hl{\(\pi\)0.5}、\hl{X-VLA}）训练与\hl{真机部署}、\hl{FastWAM 世界动作模型}、\hl{CRAVE 零训练价值建模}、flow-matching policy、\hl{RTC 实时推理}；\hl{PyTorch（DDP）}/ \hl{JAX-Flax（FSDP）}分布式训练
...
\begin{itemize}
  ...
  \item \textbf{技术栈}: Ubuntu 20.04、\hl{ROS Noetic}、\hl{MuJoCo}、\hl{Yocto / IMX6ULL}、\hl{STM32F4}、点云处理、路径规划、\hl{神通数据库}
  \item \textbf{工作内容}: 基于 \hl{MuJoCo 搭建轮式仿人机器人仿真环境}；编写机器人总控程序，实现建图、规划、避障与逆运动学；开发\hl{自研双臂与底盘电机控制系统}，完成 STM32F4 下位机、执行机构及整机联调；基于 Yocto 构建 i.MX6ULL 系统并实现 ROS 多机通信，机械臂控制频率\hl{提升 100\%}
\end{itemize}
...
\begin{itemize}
  ...
  \item 实现 \hl{AWBC / RECAP advantage} 离线策略改进流水线，AWBC 离线 MAE@1 为 \hl{0.0079}（SFT 0.0089）；完成 \hl{RECAP / RLT 在美甲操作任务}上的应用实现与训练评测链路适配
  \item 完成清华 \hl{X-VLA} 架构在自有双臂平台真机 bring-up，通过固件级 IK 将推理周期由 \hl{1.4s 压至 0.33s}
  \item 复现并适配\hl{FastWAM 世界动作模型}，完成 Task\_A1 叠衣配置、WebSocket/ROS2 真机部署链、连续夹爪支持与 preflight 安全门禁
  \item 构建 \hl{CRAVE 零训练里程碑 / 价值引擎}，从冻结视觉特征生成 milestone 与 progress/value 信号（对人工 stage-progress 相关系数 \hl{0.865}）；探索将\hl{三档 advantage 接入 AWBC}，减少人工阶段标注依赖
\end{itemize}
